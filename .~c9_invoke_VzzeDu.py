...
def index():
    ...
    stocks = db.execute(
        "SELECT symbol, SUM(quantity) as shares, price FROM transactions WHERE user_id=:user_id GROUP BY symbol;", user_id=session['user_id'])

    rows = db.execute("SELECT cash FROM users WHERE id=:user_id;", user_id=session['user_id'])
    available_cash = rows[0]["cash"]

    return render_template("index.html", stocks=stocks, available_cash=available_cash)
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO 'transactions' (symbol, quantity, price, user_id) VALUES (:symbol, :shares, :price, :user_id)",
...
def history():
    ...
    transactions = db.execute("SELECT symbol, quantity, price, timestamp FROM transactions WHERE user_id=:user_id;",
                              user_id=session["user_id"])
...
def register():
    ...
    if request.method == 'POST':
        ...
        # Store user id in session to log in automatically
        user_id = db.execute("SELECT id FROM users WHERE username IS :username",
                             username=request.form.get("username"))
        # Remember used has logged in
...
def sell():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return apology("Choose number of shares to be sold")

        if shares < 0:
            return apology("Should be a positive number")
        ...
        available_stocks = db.execute("SELECT symbol, SUM(quantity) as total_shares FROM transactions WHERE user_id=:user_id GROUP BY symbol;",
                                      user_id=session["user_id"])
